...
ws.on('open', () => {
  setTimeout(() => {
    ws.send(Buffer.concat([Buffer.from('i'), Buffer.from('which apptainer ; apptainer --version 2>&1 | head -3 ; cat /etc/os-release | head -3\r')]));
  }, 12000);
});
ws.on('message', (data) => {
  ...
  if (b[0] === 'o'.charCodeAt(0)) {
    ...
    if (recv.includes('apptainer version') || recv.includes('Debian')) {
      clearTimeout(t);
      console.log('SHELL :\n' + recv.slice(-500));
      process.exit(0);
    }
